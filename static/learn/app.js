...
async function loadCatalog() {
    const res = await fetch("/api/learn/catalog");
...
function renderStyles() {
    ...
    (state.catalog.styles || []).forEach((style) => {
        ...
        btn.innerHTML = `<img alt="" src="images/${style.id}/preview.jpg"><span>${style.ja}</span>`;
...
async function ask(query) {
    ...
    const res = await fetch("/api/learn/interest", {
        method: "POST",
...
async function loadBooks(query) {
    ...
    try {
        const res = await fetch(`/api/learn/books?q=${encodeURIComponent(query)}`);
        ...
        state.books.forEach((book) => {
            ...
            const cover = book.coverUrl
                ...
                : `<img alt="" src="images/${state.style}/book.jpg">`;
...
loadCatalog().catch(() => {
    document.getElementById("home-bubble").textContent =
        "サーバーに接続できなかったよ。Tank-OCR を起動して、/learn/ を開いてね。";
});
