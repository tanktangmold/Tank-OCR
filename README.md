...
同一地址的 <http://127.0.0.1:8000/learn/> 是给孩子用的中文小故事：先说喜欢什么（比如足球、C罗、绘本），再按一句一图阅读。字可以点着听，句子也可以点着听，画风能在漫画 / 绘本 / 写实之间切换。课文都是原创短句，不会照抄网书；相关童书只作为家长查找线索。
...
handler/
  ...
  status.go          状态
  history.go         历史记录
  learn.go           中文小故事：兴趣匹配、句型短句、相关童书
learn/               孩子中文学习库（故事、句型、找书）
static/              自带的网页界面
static/learn/        一句一图的中文阅读页（可切换画风、点词发音）
scripts/
